...
> **Backend-phase reference.** Describes the **target** full-stack MVP (auth, OpenSearch, all CRUD APIs, AWS). During the **frontend-only phase**, use [`MVP_FRONTEND_ONLY_SCOPE.md`](MVP_FRONTEND_ONLY_SCOPE.md) and [`AS_BUILT_SNAPSHOT.md`](AS_BUILT_SNAPSHOT.md) for what is built today. Do not treat baseline scope as shipped.
