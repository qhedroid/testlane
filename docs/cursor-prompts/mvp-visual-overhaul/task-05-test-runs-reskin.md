# Task 05 — Test Runs reskin (PROTECTED execution UX — visual only)
...
- `apps/web/src/fresh/components/RunStatusInfographic.tsx` / `RunDonut.tsx` (donut colours — if not
  already done in task-03; make sure both screens share the corrected constants)
...
2. **Summary donut + status list** — the always-visible status rows and donut read their colours
   ...
   (see task-03 change 2). Blocked = amber `#E4AF03`, skipped = purple `#4527A0` (unchanged), not-run
   `#BAC5CD`. Do not change which statuses render or their click-to-filter behaviour.
...
- Screenshots + explicit "protected UX behaviour verified unchanged" note in the QA report.
