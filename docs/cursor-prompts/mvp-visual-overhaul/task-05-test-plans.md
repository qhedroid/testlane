...
- (`RunDonut.tsx` colours already corrected in task-02 — reused here)

## Changes
1. **Grep `prototype-plans.css` for hardcoded status/greys** and re-point to the `--*` tokens
   (same list as task-04 change 1). Radii → `--r-*`.
...
4. **Coverage donut** — via `RunDonut` (Compass colours from task-02). Keep geometry/props.
...
## Documentation
- `docs/claude/handoff.md` — mark task-05 (Test Plans) done under `mvp-visual-overhaul`.
