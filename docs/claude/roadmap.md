...
Branch `mvp-dashboard-metrics`. Tasks 01–04 committed (`5544fc0`) — real metric cards, active-run list, needs-attention panel, coverage-by-folder panel, demo-only placeholder gate removed. Task-05 (bug-fix follow-up) drafted at `docs/cursor-prompts/mvp-dashboard-metrics/task-05-skipped-status-and-hover-tooltip.md`, not yet run: dashboard run-card donuts were folding Skipped into Not run instead of showing it as its own segment, and were missing the hover tooltip RunsScreen's/PlansScreen's donuts already have — both traced to the donut call not passing `skipped`/`interactive` the way those two screens already do.
