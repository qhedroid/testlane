...
### Dashboard Metrics `[~in progress]` — implemented, PR pending
Branch `mvp-dashboard-metrics`. All work committed: tasks 01–04 (`5544fc0`) — real metric cards, active-run list, needs-attention panel, coverage-by-folder panel, demo-only placeholder gate removed — plus task-05 bug-fix follow-up (`323ce6f`) fixing dashboard run-card donuts that were folding Skipped into Not run instead of showing it as its own segment, and were missing the hover tooltip RunsScreen's/PlansScreen's donuts already have. Ready for PR review; will flip to `[x]` once merged to `mvp-main`.
