...
**2026-07-08 Phase 2 actuals (calibration in progress):** task-07 (shell — sidebar/topbar restructure + new route stubs, solo) landed at **~48%**; task-08 (six new screens — Login, My Work, Milestones, Requirements, Reports, AI Studio, bundled) landed at **~55%**; task-09 (Dashboard rebuild — real selectors/charts wired to live `FreshProvider` state, solo, single screen) landed at **~50%**. Revised read after this third point: the earlier "bundling drives it up" theory holds better than a "real data wiring costs more per screen" theory — task-09 involved real data rewiring (heavier per-screen work than task-08's static demo shells) but, being solo, still landed below task-08's bundled 55%, close to task-07's 48% floor. Working model now: **solo single-screen Phase 2 tasks land ~48–50% regardless of whether the screen is a light shell or real data rewiring; bundling multiple screens into one session is what pushes usage up (~55% for 6 screens).** Shaun decided against bundling task-09–12 together (2026-07-08) specifically on this reasoning, plus task-12's protected-UX margin — all four run solo as originally scoped. This also lowers risk on task-13 (Defects+Audit+Project Settings, 3 screens bundled): expect it above the ~48–50% solo floor but likely at or below task-08's 55% given fewer screens; if it lands materially over ~55–58%, split it (e.g. Defects+Audit, then Settings alone). task-12 (Test Runs, protected UX) stays solo regardless of token headroom either way — budget margin doesn't reduce behavioural-regression risk on that screen.
