...
**2026-07-08 Phase 2 actuals (calibration in progress):** task-07 (shell — sidebar/topbar restructure + new route stubs, solo) landed at **~48%**, the first data point for this heavier component-authoring work — a bit above the Phase 1 pure-reskin band (41–46%), consistent with real markup/route authoring costing more per session than CSS-token retargeting. One point isn't enough to redraw the sizing model, and 48% is only modestly over the old ceiling, so no resizing yet: proceed with task-08 (six new screens, the most bundled of the remaining Phase 2 tasks) as planned, and treat it as the next critical calibration point. If task-08 lands near ~48–55%, the "fixed per-session overhead dominates" model likely still holds with a slightly higher Phase 2 baseline; if it spikes past ~60%, split task-08's six screens before running and reconsider the task-13 (Defects+Audit+Settings) bundle too. task-12 (Test Runs, protected UX) stays solo regardless of token headroom — budget margin doesn't reduce behavioural-regression risk on that screen.
