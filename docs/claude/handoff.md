...
**2026-07-08 Phase 2 actuals (calibration in progress):** task-07 (shell, solo) **~48%**; task-08 (six new screens, bundled) **~55%**; task-09 (Dashboard rebuild, real data wiring, solo) **~50%**; task-10 (Test Cases hybrid — toolbar relocation + pane restyle, solo) **~54%**. Solo-task trend is creeping up (48 → 50 → 54), not flat at ~48–50% as the task-09 read suggested — bundling still looks like the bigger lever (55% for 6 screens vs. 54% for the highest solo task so far), but per-task variance among solo tasks is wider than first thought, so don't treat 48–50% as a hard solo floor anymore. Practical read for what's left: task-11 (Test Plans, solo) likely similar range (~50–55%); task-12 (Test Runs, protected UX, solo) is the one to watch most closely — it's both the most structurally complex remaining screen (three-pane workspace) and the one where a token-thin session is riskiest, since the protected-UX regression check (auto-advance, P/F/B/S + arrow keys, `/runs/api`) needs full attention regardless of where in the session it happens. If task-12 reports high usage, that's a signal to keep the *regression check* thorough even if it means the visual polish is left rougher than other screens — never the reverse. task-13 (Defects+Audit+Project Settings, 3 screens bundled) still expected at/below task-08's 55% given fewer screens; split it (e.g. Defects+Audit, then Settings alone) if it lands materially over ~55–58%.
