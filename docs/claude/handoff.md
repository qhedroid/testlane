...
**2026-07-08 Phase 2 actuals (calibration in progress):** task-07 (shell, solo) **~48%**; task-08 (six new screens, bundled) **~55%**; task-09 (Dashboard rebuild, real data wiring, solo) **~50%**; task-10 (Test Cases hybrid — toolbar relocation + pane restyle, solo) **~54%**; task-11 (Test Plans — full layout rebuild, `resolvePlanCases()`/query logic preserved, solo) **~47%**. Five points in, solo tasks range **47–54%** with no clear monotonic trend (48, 50, 54, 47) — read this as per-task variance within a band, not a rising trajectory. Bundling (task-08, 55%) still sits at or above the top of that solo band, so the original "bundling is the bigger lever" read holds. Practical implication for task-12 (Test Runs, protected UX, solo, the most structurally complex remaining screen — three-pane workspace): the 47–54% solo band gives reasonable confidence it won't run so token-thin that the protected-UX regression check (auto-advance, P/F/B/S + arrow keys, `/runs/api`) gets shortchanged, but the standing instruction stays regardless — if usage does run high, prioritize the regression check over visual polish, never the reverse. task-13 (Defects+Audit+Project Settings, 3 screens bundled) expected in the mid-to-high side of the solo band or slightly above (fewer screens than task-08's 55%); split it (e.g. Defects+Audit, then Settings alone) if it lands materially over ~55–58%.
