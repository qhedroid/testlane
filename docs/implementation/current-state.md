...
| TestRunService design | Reference implementation in `packages/db/services/TestRunService.ts` (not wired to app) |
...
- `TestRunService` integration into API routes and app runtime
...
| **Service wiring** | `TestRunService.ts` references paths (`../db`, `../opensearch/client`) that do not match monorepo layout yet — reference code only |
...
# 4. App health (requires pnpm dev in another terminal)
curl http://localhost:3000/api/health
...
**TestRunService.create() integration and validation.**

The local seed layer is **implemented** (`pnpm db:seed`). The dataset exists to support the next phase. If your machine has not been seeded yet, run `pnpm db:seed` before service work.

### Purpose of the next phase

- Wire `TestRunService.create()` to the monorepo DB client and runtime dependencies
- Validate spawn transaction end-to-end against seeded CTMS plan `PLAN-001`
- Confirm RBAC gate (`admin` or `super_admin`) and `ref_counters` behaviour under local MySQL
- Establish the first real write-path integration test beyond health check

### Seed reference (for TestRunService.create)

From `packages/db/src/seed/ids.ts`:

```
projectId:  01SEED00000000000000000010   # CTMS
testPlanId: 01SEED00000000000000000400   # PLAN-001 (active, 4 cases)
createdBy:  01SEED00000000000000000003   # Shaun Sevume (admin)
```

### Explicitly deferred

**UI implementation remains intentionally out of scope.** Do not build Dashboard, Cases, Plans, or Runs screens until the service layer and API contracts for run spawning are validated.
